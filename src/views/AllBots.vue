...
// import BotTargetFilter from '@/components/BotTargetFilter'
...
export default {
  ...
  methods: {
    ...
    loadBots () {
      ...
      this.axios.get(`/api/bots?${statusFilter}&filter[target]=${this.filter.target}&filter[since]=${this.filter.since}&page[number]=${this.pagination.page}&page[size]=${this.pagination.size}&sort=${this.sort}`)
        .then(response => {
