  ...
  <div>
    <div class="rounded border bg-white p-3">
      <b-form
        @submit="onSubmit"
      >
        <h4>Update Github installations</h4>
  ...
  methods: {
    ...
    loadForm () {
      return this.axios.get('/api/admin/integrations')
        .then(response => {
          if (response.data && response.data.github && response.data.github.apps) {
            response.data.github.apps.forEach(app => {
              if (app.installations) {
                app.installations.forEach(installation => {
                  if (installation && `${installation.id}` === this.id) {
