  ...
  <div>
    ...
    <div v-if="pipeline" class="row rounded border pt-3 pr-2 pb-2 pl-2 mt-2 mr-3 mb-2 ml-3" :class="pipeline.buildStatus | bootstrapClass('border')">
      ...
      <div class="mb-2 col-6 col-md-4 col-xl-1 align-middle">
        ...
        <router-link :to="{ name: 'PipelineBuildLogs', params: { repoSource: pipeline.repoSource, repoOwner: pipeline.repoOwner, repoName: pipeline.repoName, repoRevision: pipeline.repoRevision }}" tag="div" class="progress mt-2 clickable">
      ...
      <div v-if="pipeline.labels && pipeline.labels.length > 0" class="col-12"><div class="mt-3 mb-3 w-50 mx-auto border-bottom"></div></div>
      <div v-if="pipeline.labels && pipeline.labels.length > 0" class="mb-2 col-12 text-center text-truncate text-truncate-fade">
